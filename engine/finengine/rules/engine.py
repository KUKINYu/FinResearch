"""财务异常检测规则引擎（M5 核心卖点；P1 加规则开关与阈值可配置）。

设计原则：
- 规则即数据：每条规则 = 人话标题 + 说明 + 严重程度 + 可调参数（默认阈值）
- 每条异常必须输出：结论（人话）+ 计算过程（中文公式与数据明细）+
  涉及数据点（含出处页码）——"不只是告诉你存在异常，还告诉你
  由哪些数据算出、来自原文哪一页"
- 检查基于整年期间（1-6月期间数不参与同比）
- P1：阈值与开关存数据库（settings 表 rules.config），界面可调
...
RULE_DEFS: list[tuple[str, str, str, Callable]] = []
# 每条规则的参数：name → (默认值, 人话标签)
RULE_PARAMS: dict[str, dict[str, tuple[float, str]]] = {}


def rule(rule_id: str, title: str, severity: str):
    """注册规则（装饰器）。"""

    def deco(fn: Callable[[Series, dict], list[dict]]):
...
def param(rule_id: str, name: str, default: float, label: str):
    """注册规则参数（在 rule 装饰之后调用）。"""

    def deco(fn):
        RULE_PARAMS.setdefault(rule_id, {})[name] = (default, label)
        return fn

    return deco


def _get(rule_id: str, p: dict, name: str) -> float:
    """取参数值：用户覆盖 > 默认值。"""
    if name in p:
        return float(p[name])
    return RULE_PARAMS.get(rule_id, {}).get(name, (0.0, ""))[0]


# ---- 规则实现 ----

@rule("ar_vs_revenue_growth", "应收账款增速明显快于营业收入", "high")
@param("ar_vs_revenue_growth", "diff_pp", 20.0, "增速差阈值（百分点）")
def r_ar_vs_revenue(s: Series, p: dict) -> list[dict]:
    """应收账款增速 - 营业收入增速 > 阈值。"""
    diff_pp = _get("ar_vs_revenue_growth", p, "diff_pp")
    out = []
        ...
        if diff > diff_pp:
            out.append(
                make_anomaly(
                    ...
                    f"增速差 {diff:.1f} 个百分点，超过 {diff_pp:.0f} 个百分点的关注阈值，"
...
@param("inventory_vs_revenue_growth", "diff_pp", 20.0, "增速差阈值（百分点）")
def r_inventory_vs_revenue(s: Series, p: dict) -> list[dict]:
    """存货增速 - 营业收入增速 > 阈值。"""
    diff_pp = _get("inventory_vs_revenue_growth", p, "diff_pp")
    out = []
    inv = annual(s.get("存货", {}))
    rev = annual(s.get("营业收入", {}))
    for year in inv:
...
@param("profit_vs_ocf_growth", "np_growth_min", 20.0, "净利润增速下限（%）")
@param("profit_vs_ocf_growth", "lag_pp", 20.0, "现金流落后阈值（百分点）")
def r_profit_vs_ocf(s: Series, p: dict) -> list[dict]:
    """净利润增速 > 下限，且经营现金流增速落后 > 阈值（或现金流下滑）。"""
    np_min = _get("profit_vs_ocf_growth", p, "np_growth_min")
    lag_pp = _get("profit_vs_ocf_growth", p, "lag_pp")
    out = []
    ...
    for year in np_:
        ...
        if np_g is None or np_g <= np_min or ocf_g is None:
            continue
        if ocf_g < np_g - lag_pp:
            ...
            out.append(
                make_anomaly(
                    ...
                    f"两者差距超过 {lag_pp:.0f} 个百分点，利润的现金含量下降，"
...
@param("ocf_np_divergence", "ratio_max", 0.5, "现金流/净利润阈值（连续两年低于）")
def r_ocf_np_divergence(s: Series, p: dict) -> list[dict]:
    """连续两年 经营现金流/净利润 < 阈值（或现金流为负）。"""
    ratio_max = _get("ocf_np_divergence", p, "ratio_max")
    np_ = annual(s.get("净利润", {}))
    ocf = annual(s.get("经营活动产生的现金流量净额", {}))
    ...
    if len(bad_years) >= 2 and bad_years[-1][0] - bad_years[-2][0] == 1:
        ...
        return [
            make_anomaly(
                ...
                f"连续两年低于 {ratio_max:.1f} 的阈值（经营现金流 "
...
@param("gross_margin_change", "change_pp", 5.0, "同比变动阈值（百分点）")
def r_gross_margin_change(s: Series, p: dict) -> list[dict]:
    """毛利率同比变动超过阈值。"""
    change_pp = _get("gross_margin_change", p, "change_pp")
    out = []
    gm = annual(s.get("毛利率", {}))
    for year in gm:
        prev = year - 1
        ...
        if abs(diff) > change_pp:
            ...
            out.append(
                make_anomaly(
                    ...
                    f"超过 {change_pp:.0f} 个百分点的关注阈值。需关注产品结构变化、原材料价格、"
                    f"定价策略或成本核算口径变化。",
                    [
...
@param("debt_ratio_change", "change_pp", 10.0, "同比变动阈值（百分点）")
def r_debt_ratio_change(s: Series, p: dict) -> list[dict]:
    """资产负债率同比变动超过阈值。"""
    change_pp = _get("debt_ratio_change", p, "change_pp")
    out = []
        ...
        if abs(diff) > change_pp:
            ...
            out.append(
                make_anomaly(
                    ...
                    f"超过 {change_pp:.0f} 个百分点的关注阈值。需关注融资结构、大额借款或分红等重大变化。",
                    [
...
@param("interest_debt_cash_coverage", "ratio_min", 1.2, "有息负债/货币资金阈值")
def r_interest_debt_cash(s: Series, p: dict) -> list[dict]:
    """最近一年 有息负债/货币资金 > 阈值。"""
    ratio_min = _get("interest_debt_cash_coverage", p, "ratio_min")
    ...
    if ratio is not None and ratio > ratio_min:
        return [
            make_anomaly(
                ...
                f"有息负债/货币资金 = {ratio:.2f}，超过 {ratio_min:.1f} 的阈值，"
...
@param("rd_ratio_change", "change_pp", 5.0, "占比变动阈值（百分点）")
def r_rd_ratio_change(s: Series, p: dict) -> list[dict]:
    """研发费用/营业收入 同比变动超过阈值。"""
    change_pp = _get("rd_ratio_change", p, "change_pp")
    out = []
        ...
        if abs(diff) > change_pp:
            ...
            out.append(
                make_anomaly(
                    ...
                    f"{direction} {abs(diff):.1f} 个百分点（阈值 {change_pp:.0f} 个百分点）。"
...
@param("revenue_vs_cash_received", "ratio_min", 0.7, "收现比阈值（低于触发）")
def r_revenue_vs_cash(s: Series, p: dict) -> list[dict]:
    """最近一年 销售商品提供劳务收到的现金/营业收入 < 阈值。"""
    ratio_min = _get("revenue_vs_cash_received", p, "ratio_min")
    rev = annual(s.get("营业收入", {}))
    ...
    if ratio is not None and ratio < ratio_min:
        return [
            make_anomaly(
                ...
                f"收现比 = {ratio:.2f}，低于 {ratio_min:.1f} 的阈值。"
...
@param("ar_days_deterioration", "growth_pct", 30.0, "周转天数拉长阈值（%）")
def r_ar_days(s: Series, p: dict) -> list[dict]:
    """应收账款周转天数同比增长超过阈值。"""
    growth_pct = _get("ar_days_deterioration", p, "growth_pct")
    out = []
    ar = annual(s.get("应收账款", {}))
    rev = annual(s.get("营业收入", {}))
    for year in ar:
        ...
        if g > growth_pct:
            out.append(
                make_anomaly(
                    ...
                    f"上年约 {days_prev:.0f} 天，拉长 {g:.0f}%，超过 {growth_pct:.0f}% 的阈值。"
...
# ---- 规则设置（P1：开关 + 阈值，存 settings 表） ----

def load_rule_settings() -> dict[str, dict]:
    """读取规则设置：{rule_id: {enabled: bool, params: {name: value}}}。"""
    import json
    ...
    with SessionLocal() as session:
        row = session.get(Setting, "rules.config")
...
def save_rule_settings(settings: dict[str, dict]) -> None:
    """保存规则设置（覆盖式）。"""
    import json
    ...
    with SessionLocal() as session:
        ...
        if row is None:
            session.add(Setting(key="rules.config", value=payload))
        else:
            row.value = payload
        session.commit()
...
def describe_rules() -> list[dict]:
    """规则清单（供设置界面）：标题、说明、参数与当前值。"""
    saved = load_rule_settings()
    out = []
    for rule_id, title, severity, _fn in RULE_DEFS:
        params = []
        for name, (default, label) in RULE_PARAMS.get(rule_id, {}).items():
            value = (saved.get(rule_id, {}).get("params", {}).get(name, default))
            params.append({"name": name, "label": label, "default": default, "value": value})
        out.append(
            {
                "rule_id": rule_id,
                "title": title,
                "severity": severity,
                "enabled": saved.get(rule_id, {}).get("enabled", True),
                "params": params,
            }
        )
    return out
...
def run_rules(series: Series, settings: dict[str, dict] | None = None) -> list[dict]:
    """对项目指标序列运行全部规则（可传设置覆盖；None = 读库）。"""
    if settings is None:
        settings = load_rule_settings()
    anomalies: list[dict] = []
    for rule_id, title, severity, fn in RULE_DEFS:
        cfg = settings.get(rule_id, {})
        if not cfg.get("enabled", True):
            continue
        params = cfg.get("params", {})
        try:
            anomalies.extend(fn(series, params))
