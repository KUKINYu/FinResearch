...
- [x] M8 打包发布准备（引擎 exe 重建 81.8MB 含全部依赖、安装包 189.7MB、打包成品验收通过；README + Apache-2.0 LICENSE 就绪。**待创始人操作：注册 GitHub → 建仓库 → 推送 → 创建 Release 上传安装包**）

## P0 完成状态（2026-09-24）
全部功能里程碑 M1-M8 完成：五个工作区（项目档案/财务指标+校对+趋势图/异常溯源/全文搜索/AI 问答）全部可用，50 个单元测试通过，打包成品验证通过。

## P1 进度（2026-09-25 全部完成）
- [x] P1-1 同行对比：A 股数据源（akshare，统一接口+market_cache 缓存 24h/1h+失败降级）、可比公司管理、对比表+柱状图、导出 Excel
- [x] P1-2 成果导出：异常清单（Word，含数据明细表与出处页码）、指标表（Word）、对比表（Excel）
- [x] P1-3 异常规则自定义：10 条规则全部参数化（开关+阈值），存 settings 表 rules.config，界面设置面板保存后自动重新分析
- [x] P1-4 研究记忆与反思（替代原 OCR 计划，按创始人要求）：数据更新自动笔记（新文件解析后对比旧数据生成结构化差异，无需 AI）+ AI 反思（历史笔记+异常 → 快模型总结）+ 手写笔记
- [x] P1-5 公告与舆情监控：akshare 公告+个股新闻、负面词识别（诉讼/处罚/问询等）、时间分层（近3天/近30天/更早）、AI 快摘要；风险评分卡：五维打分（财务/现金流/偿债/营运/外部）+雷达图+依据可溯源
- [x] P1-6 MCP 服务器：fastmcp，7 个工具（list_projects/get_indicators/get_anomalies/search_documents/ask_question/get_risk_score/analyze_document），接入 Claude Desktop/Cursor；run-mcp.bat/sh
- [x] P1-7 双模型分层：深度任务 + 快速任务（摘要类）分开配模型，省 token；quick 未配置自动回落
- [x] P1-8 股价走势图：近一年前复权日线（akshare，6h 缓存）
- [x] P1-9 数据源自动回退链：akshare（东财）失败自动切 baostock（已安装），行情与股价双路
- [x] 无头分析 CLI 落地：`finengine analyze <文件>` 实测解析鸿富诚招股书 → 指标带页码 + 5 条异常
- 修复：PDF 出处渲染（pdf.js worker 路径在 dev 模式 404 → ?url 导入）；批处理脚本 GBK+CRLF（影响所有用户安装）；桌面快捷方式（launch-silent.vbs 从注册表取路径，纯 ASCII 防编码坑）
- 网络备注：2026-09-24 本机网络对 HF/eastmoney 接口不稳定。已做：模型/行情缓存 + 优雅降级 + baostock 备用源。59 个单元测试全部通过。
