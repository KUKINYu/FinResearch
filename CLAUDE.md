...
- [ ] M3 阅读器+校对 → M4 财务提取 → M5 异常溯源 → M6 搜索 → M7 AI 问答 → M8 打包发布

## 已知数据质量问题（待后续里程碑处理）
- **子公司数据污染**：招股书子公司章节的多期表格会被提取为项目指标（实测中塑第55页"净利润2022年度273.44万"是苏州宝利金子公司的数据）。处理：M3 校对界面让用户删除错误行；M4 加口径过滤（摘要表需 ≥3 年列、排除含"子公司"上下文的表格）。
